...
class VirtualFile:
    ...
    def getChild(self, name: str) -> Optional["VirtualFile"]:
...
""".strip())
    ...
    def display(self, node: Optional[VirtualFile] = None, indent: int = 0, order: int = 0):
        ...
        commands: dict[str, Any] = {
            f"{order}": lambda: (run(os.path.join(node.path, "__Content__")), print("Modification Commited."))
        }
        if indent != 0:
        ...
        for child_name in node.getChildren():
            ...
            if child:
                new_order, new_commands = self.display(child, indent + 1, order + 1)
                order += new_order
                commands |= new_commands
        print("  " * indent + f"  add child [a{indent}]")
        commands[f"a{indent}"] = lambda: (run(os.path.join(self.addChild(node, input("Child Name: ", ), select("Child Type:", [
            "Script",
            "Module",
            "Folder",
            "Package",
